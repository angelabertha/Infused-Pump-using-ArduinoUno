...
void loop() {
  if (running) {
    ...
    if (key == 'C') { stopSystem("STOP ditekan!"); return; }
    else if (key == 'D') { stopSystem("RESET ditekan!"); return; }
    ...
    if (remainingTime <= 0) stopSystem("Infus selesai!");
...
void inputMenu() {
  lcd.clear(); lcd.setCursor(0, 0); lcd.print("Volume (ml):");
  targetVolume = inputNumber();

  lcd.clear(); lcd.setCursor(0, 0); lcd.print("Waktu (mnt):");
